...
export default function AccountPage() {
  ...
  async function carica() {
    ...
    const { data: profilo } = await supabase
      .from("profili")
      .select("nome, cognome, impresa, nome_utente, attivita_base")
      ...
      <div style={{ padding: 16, border: "1px solid #ddd", borderRadius: 8, marginBottom: 20 }}>
        ...
        <form onSubmit={salvaProfilo}>
          <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
            ...
            <div style={{ flex: 1 }}>
              <label style={{ display: "block", fontSize: 13, marginBottom: 4 }}>Cognome</label>
              <input value={cognome} onChange={(e) => setCognome(e.target.value)} style={{ width: "100%", padding: 8 }} />
            </div>
          </div>
          <p style={{ fontSize: 12, color: "#888", marginTop: -4, marginBottom: 12 }}>
            Servono per riconoscerti nel registro presenze del cantiere.
          </p>
          ...
          <div style={{ marginBottom: 8 }}>
            <label style={{ display: "block", fontSize: 13, marginBottom: 4 }}>Attività</label>
            <input value={attivitaBase} onChange={(e) => setAttivitaBase(e.target.value)} style={{ width: "100%", padding: 8 }} />
          </div>
